...
scratcardInstances = [1 for _ in range(playdf.shape[0])]
...
for row_index in range(windf.shape[0]):
    ...
    # print(f'row index {row_index} has # of wins: {count}')
    for i in range(count):
        if row_index + i + 1 < len(scratcardInstances):
            scratcardInstances[row_index+i+1] += scratcardInstances[row_index]
    # print(scratcardInstances)
print(scratcardInstances)
print(sum(scratcardInstances))
